...
export const NavBar = () => {
  ...
  return (  
    ...
    <div ref={navBarRef}  className={`   z-20 top-0  h-[81px]     flex w-full max-w-screen-2xl justify-between items-center ${isScrolling ? ' sticky transform transition-all duration-500 rounded-3xl  top-0 shadow-2xl bg-[#9a9a9a]' : 'absolute'} `}>
        <img src={logo} alt="logo" className={`  ml-3.5  ${isScrolling ? 'size-22 mt-0 transform transition-all duration-400 ' : 'size-30 mt-8'} `} /> 
        ...
        {/*for mobile */}
        <button className='lg:hidden flex mr-2' onClick={()=>setIsMenuClicked(!isMenuClicked)}> <CiMenuBurger size={'25px'} /></button>
        {isMenuClicked?
        <div className=' w-full'>
        <div className=' text-white rounded-4xl   pt-6 w-1/2 h-screen absolute top-0 right-0 flex flex-col space-y-3 shadow-2xl bg-white '>   
        ...
        <div ref={overlayRef} className='w-1/2 h-screen absolute top-0 left-0 bg-[#575756] opacity-60 '>
